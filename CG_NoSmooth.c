...
{ 
  ...
  if(inall->rep == 0){   /* no assignment or stor alloc for bootstrap reps */
    ...
    if(method == 'M' || method == 'L'){            /* method == 'M' or 'L' */
      ...
      inall->nm = 2;
      for(i=4;i<=5;i++) strcpy(inall->names[i-4],names[i]);
    }
    else if(method == 'F'){                               /* method == 'F' */
      inall->nm = 2;
      for(i=6;i<=7;i++) strcpy(inall->names[i-6],names[i]);
    }
    else if(method == 'G'){                               /* method == 'G' */
      inall->nm = 4;
      for(i=4;i<=7;i++) strcpy(inall->names[i-4],names[i]);
    }
    else if(method == 'C'){                               /* method == 'C' */
      inall->nm = 1;
      strcpy(inall->names[0],names[8]);
    }
	else if(method == 'H'){                               /* method == 'H' */
      inall->nm = 3;
	  for(i=4;i<=5;i++) strcpy(inall->names[i-4],names[i]);
      strcpy(inall->names[2],names[8]);
    }
    else{                                                 /* method == 'A' */
      inall->nm = 5;
      for(i=4;i<=8;i++) strcpy(inall->names[i-4],names[i]);
    }                                                               
...
*/
  ...
  if(inall->method == 'M' || inall->method == 'L'){
    fprintf(fp,"%s Equating with CINEG Design (%s Anchor; w1 = %7.5f)\n\n",
    ...
    fprintf(fp,"                  Method 0:   Method 1:   Method 2:   Method 3:\n");
    fprintf(fp,"                 %s  %s  %s  %s\n\n",
                inall->names[0],inall->names[1],inall->names[2],inall->names[3]);
    fprintf(fp,"          msx  %12.5f%12.5f\n",r->msx[0],r->msx[1]);
    fprintf(fp,"          msy  %12.5f%12.5f\n",r->msy[0],r->msy[1]);
          
    if(inall->method != 'M'){        
      fprintf(fp,"          ssx  %12.5f%12.5f\n",r->ssx[0],r->ssx[1]);
      fprintf(fp,"          ssy  %12.5f%12.5f\n",r->ssy[0],r->ssy[1]);
    }
          
    fprintf(fp,"\n       gamma1  %12.5f%12.5f%12.5f%12.5f\n",
            r->gamma1[0],r->gamma1[1],r->gamma1[2],r->gamma1[3]);
    fprintf(fp,"       gamma2  %12.5f%12.5f%12.5f%12.5f\n\n",
            r->gamma2[0],r->gamma2[1],r->gamma2[2],r->gamma2[3]);
    fprintf(fp,"      inter b  %12.5f%12.5f%12.5f%12.5f\n",
            r->b[0],r->b[1],r->b[2],r->b[3]);
    fprintf(fp,"      slope a  %12.5f%12.5f%12.5f%12.5f\n",
    ...
    if(inall->method == 'E' || inall->method == 'G' || inall->method == 'A')
      fprintf(fp,"Braun-Holland (BH) Under Frequency Estimation (FE):\n"
