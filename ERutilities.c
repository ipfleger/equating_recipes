...
*/
{
  ...
  str[end-begin+1] = '\0';
  for(i=begin;i<=end;i++) 
    if((str[i-begin] = fgetc(fp)) == EOF) return EOF;    
  *x = atof(str);
  return end-begin+1;   
}
...
*/
{
  ...
  ftp = fopen(tname,"w");
  if (ftp==NULL) runerror("No file open\n"); 
...
*/ 
{
  int xi = (int) x,    /* truncated (integer) value for x */
      max = ns-1;
  double value;
...
*/
{
  ...
  for(;;) {
    for(i=1;i<=scol-1;i++) if(fscanf(fp,"%*s") == EOF) break;
    if(fscanf(fp,"%lf",&x) == EOF) break;  
    n++;
    if(x<low) low = x;
    if(x>high) high = x;
    mean += x;
...

{
  ...
  for(;;) {
    ...
    if(x<min){
      printf("In file %s, for person %d, score = %lf",fname,n+1,x);
      runerror(" < min");
    } 
    if(x>max) {
      printf("In file %s, for person %d, score = %lf",fname,n+1,x);
  ...
  for(i=0;i<=loc(max,min,inc);i++) s->rfd[i] = (double) s->fd[i]/n;
...
*/
{
  ...
  for(;;) {
    ...
    for(i=1;i<=onec-1;i++) 
      if(fscanf(fp,"%*s") == EOF) break;    /* skip to 1st score in record */             
    if(fscanf(fp,"%lf",(cols>rows) ? &one : &two) == EOF) break; /* row sc */
    
    for(i=1;i<=twoc-onec-1;i++) 
      if(fscanf(fp,"%*s") == EOF) break;    /* skip to 2nd score in record */  
    if(fscanf(fp,"%lf",(cols>rows) ? &two : &one) == EOF) break; /* col sc */

    if(one<rmin){
      printf("In file %s, for person %d, row score = %f",fname,n+1,one);
      runerror(" < rmin");
    }  
    if(one>rmax){
      printf("In file %s, for person %d, row score = %f",fname,n+1,one);
    ...
    if(two<cmin){
      printf("In file %s, for person %d, col score = %f",fname,n+1,two);
      runerror(" < cmin");
    }  
    if(two>cmax){
      printf("In file %s, for person %d, col score = %f",fname,n+1,two);
  ...
  if((s->n = MomentsFromFD(s->min1, s->max1, s->inc1, NULL,
      s->fd1, s->mts1)) != n)
      runerror("\nError somewhere in ReadRawGet_BSTATS()");
  if((s->n = MomentsFromFD(s->min2, s->max2, s->inc2, NULL,
      s->fd2, s->mts2)) != n)
      runerror("\nError somewhere in ReadRawGet_BSTATS()");
  ...
  for(i=0;i<=loc(rmax,rmin,rinc);i++) s->rfd1[i] = (double) s->fd1[i]/n;
  ...
  for(i=0;i<=loc(cmax,cmin,cinc);i++) s->rfd2[i] = (double) s->fd2[i]/n;
  ...
  for(v=0;v<=s->ns2-1;v++)               /* v indexes second variable here */                                            /* v external to x */
    for(i=0;i<=s->ns1-1;i++){
      s->bp12[i][v] = (double) s->bfd[i][v]/s->n; 
...
*/
{
  ...
  /* rounding */
   ...
   for(i=0;i<=heraw;i++){
     if(round){ 
       essr[i] = pow((double)(10.), (double)(round-1))*
                 ((int) (essu[i]/pow((double)(10.), (double)(round-1)) +.5));
...
*/
{
  ...
  fprintf(fp,"Name of file containing yct[][]: %s\n\n",inall->nameyct);
    
  fprintf(fp,"Minimum raw score in base form conversion, Y = %12.5f\n",inall->minp);
  fprintf(fp,"Maximum raw score in base form conversion, Y = %12.5f\n\n",inall->maxp);
  if(inall->round>0){
    fprintf(fp,"Lowest possible rounded scale score = %6d\n",inall->lprss);
    fprintf(fp,"Highest possible rounded scale score = %6d\n\n",inall->hprss);
  }
  ...
  for(k=1;k<=((inall->round>0)?2:1);k++){         /* beginning of loop for k */
    ...
    if(k==1) fprintf(fp,"Equated Scale Scores (unrounded)\n");
    else fprintf(fp,"Equated Scale Scores (rounded)\n");
    ...
    for(i=0;i<=loc(inall->max,inall->min,inall->inc);i++){
	  ...
	  for(j=0;j<=inall->nm-1;j++){ 
	    if(k==1) fprintf(fp,"%12.5f",s->essu[j][i]);
		else fprintf(fp,"%12.0f",s->essr[j][i]);
	  }
    }
  
	if (inall->fdx != NULL) {
      fprintf(fp,"\n\n         Mean  ");
      for(j=0;j<=inall->nm-1;j++) fprintf(fp,"%12.5f",
...
*/
{
  ...
  fprintf(fp,"Number of Persons = %6d",s->n);
  ...
  fprintf(fp," Min score for fd1[] = %12.5f\n",s->min1);
  fprintf(fp," Max score for fd1[] = %12.5f\n\n",s->max1);  
  fprintf(fp,"       Score  Freq  CFreq    Rel Freq   Cum RFreq   Perc Rank\n\n");
  for(i=0;i<=s->ns1-1;i++) 
  ...
  fprintf(fp," Min score for fd2[] = %12.5f\n",s->min2);
  fprintf(fp," Max score for fd2[] = %12.5f\n\n",s->max2);  
  fprintf(fp,"       Score  Freq\n\n");
  fprintf(fp,"       Score  Freq  CFreq    Rel Freq   Cum RFreq   Perc Rank\n\n");
  for(i=0;i<=s->ns2-1;i++) 
  ...
  if(pbfd){ 
    ...
    fprintf(fp,"        Loc |");
    for(j=0;j<=s->ns2-1;j++)  fprintf(fp,"%8d",j);
    fprintf(fp," |\n");
    fprintf(fp,"Loc   Score |");
    for(j=0;j<=s->ns2-1;j++)  fprintf(fp,"%8.2f",score(j,s->min2,s->inc2));
    fprintf(fp," |   fd1[]\n");
    fprintf(fp,"-------------");
    for(j=0;j<=s->ns2-1;j++)  fprintf(fp,"--------");
    fprintf(fp,"----------\n");
  
    for(i=0;i<=s->ns1-1;i++){
      fprintf(fp,"%3d%8.2f |",i,score(i,s->min1,s->inc1));
      for(j=0;j<=s->ns2-1;j++)  fprintf(fp,"%8d",s->bfd[i][j]);
      fprintf(fp," |%8d\n",s->fd1[i]);
    } 
    ...
    fprintf(fp,"-----------\n");
    fprintf(fp,"      fd2[] |");
    for(j=0;j<=s->ns2-1;j++)  fprintf(fp,"%8d",s->fd2[j]);
    fprintf(fp," |%8d",s->n);
    fprintf(fp,"\n\n");
    ...
    fprintf(fp,"-----------\n");
    fprintf(fp,"     rfd2[] |");
    for(j=0;j<=s->ns2-1;j++)  fprintf(fp,"%8.5f",(double) s->fd2[j]/s->n);
    fprintf(fp," |%8.5f",grandsum);
    fprintf(fp,"\n\n");
...
*/
{
  ...
  fprintf(fp,"\n\n%s",label);
  fprintf(fp,"\n\n");
  fprintf(fp,"%s  ",rowhead);
  fprintf(fp,"%s",colhead);
  fprintf(fp,"\n");
  for(i=0;i<nrows;i++)
    fprintf(fp,"\n%12d  %12.5f",i,vector[i]);
  fprintf(fp,"\n\n");
  for(i=1;i<=26;i++) fprintf(fp,"-");
...
*/
{
  ...
  fprintf(fp,"\n\n");
  fprintf(fp,"%s  ",rowhead);
  fprintf(fp,"%s",colheads);
  fprintf(fp,"\n");
  for(i=0;i<nrows;i++){
    fprintf(fp,"\n%18d  ",i);
    for(j=0;j<ncols;j++) 
      fprintf(fp,"%18.5f",matrix[i][j]); 
  }
  fprintf(fp,"\n\n");
  for(i=1;i<=18*(ncols+1)+2;i++) fprintf(fp,"-");
}
...
*/
{
  ...
  fprintf(fp,"\n\n");
  for(i=1;i<=40;i++) fprintf(fp,"-");
  fprintf(fp,"\n");
  while( (c = fgetc(inf)) != EOF) fputc(c,fp);
  for(i=1;i<=40;i++) fprintf(fp,"-");
  fprintf(fp,"\n");
 ...
 */
{
	...
	{
		for(j=0; j<ncol;j++)
			printf("%f ",matrix[i+offset][j+offset]);
		printf("\n");
	}
 ...
 */
{
	...
	case CHARACTER:
		matrixc = (char **)matrix;	
		free(matrixc[rid_low]+cid_low-1);
		free(matrixc+rid_low-1);
		break;
	case INTEGER:
		matrixi = (int **)matrix;	
		free(matrixi[rid_low]+cid_low-1);
		free(matrixi+rid_low-1);
		break;
	case FLOAT:
		matrixf = (float **)matrix;	
		free(matrixf[rid_low]+cid_low-1);
		free(matrixf+rid_low-1);
		break;
	case DOUBLE:
		matrixd = (double **)matrix;	
		free(matrixd[rid_low]+cid_low-1);
		free(matrixd+rid_low-1);
		break;
	default:
 ...
 */
{
	...
	case CHARACTER: 
		...
		matrixc=(char **) malloc((size_t)((nrow+1)*sizeof(char*)));
		if (!matrixc) 
		...
		matrixc[rid_low]=(char *) malloc((size_t)((nrow*ncol+1)*sizeof(char)));
		/* adjust pointers to point proper row */ 
		if (!matrixc[rid_low]) 
			er_error2("allocate_matrix",
			"can not allocate memory for char elements"); 
		/* set values of vector to 0.0 */
		memset(matrixc[rid_low],0,(nrow*ncol+1)*sizeof(char));
		matrixc[rid_low] += 1;
	...
	case INTEGER: 
		/* allocate memory for row vector pointers */
		matrixi=(int **) malloc((size_t)((nrow+1)*sizeof(int*)));
		if (!matrixi) 
			er_error2("allocate_matrix",
		...
		matrixi -= rid_low; 
		/* allocate memory for whole matrix    */
		matrixi[rid_low]=(int *) malloc((size_t)((nrow*ncol+1)*sizeof(int)));
		/* adjust pointers to point proper row */ 
		if (!matrixi[rid_low]) 
			er_error2("allocate_matrix",
			"can not allocate memory for int elements");
		/* set values of vector to 0.0 */
		memset(matrixi[rid_low],0,(nrow*ncol+1)*sizeof(int));
		matrixi[rid_low] += 1;
	...
	case FLOAT:
		/* allocate memory for row vector pointers */
		matrixf=(float **) malloc((size_t)((nrow+1)*sizeof(float*)));
		if (!matrixf) 
			er_error2("allocate_matrix",
		...
		matrixf -= rid_low; 
		/* allocate memory for whole matrix    */
		matrixf[rid_low]=(float *) malloc((size_t)((nrow*ncol+1)*sizeof(float)));
		/* adjust pointers to point proper row */ 
		if (!matrixf[rid_low]) 
			er_error2("allocate_matrix",
			"can not allocate memory for float elements"); 
		/* set values of vector to 0.0 */
		memset(matrixf[rid_low],0,(nrow*ncol+1)*sizeof(float));
		matrixf[rid_low] += 1;
	...
	case DOUBLE: 
		/* allocate memory for row vector pointers */
		matrixd=(double **) malloc((size_t)((nrow+1)*sizeof(double*)));
		if (!matrixd) 
			er_error2("allocate_matrix",
		...
		matrixd -= rid_low; 
		/* allocate memory for whole matrix    */
		matrixd[rid_low]=(double *) malloc((size_t)((nrow*ncol+1)*sizeof(double)));
		/* adjust pointers to point proper row */ 
		if (!matrixd[rid_low]) 
			er_error2("allocate_matrix",
			"can not allocate memory for double elements");
		/* set values of vector to 0.0 */
		memset(matrixd[rid_low],0,(nrow*ncol+1)*sizeof(double));
		matrixd[rid_low] += 1;
 ...
 */
{
	...
	for (k=1; k<n; k++)
	{
		...
		{ 
			fprintf(stderr,"Equating Recipes error occured\n");
			fprintf(stderr,"Source: %s at 1, Error: %s \n","er_ludcmp","singular matrix");
			fprintf(stderr,"k = %d \n",k);
			fprintf(stderr,"Exiting the system. Good bye!\n");
			exit(EXIT_FAILURE);
		}
	...
	{ 
		fprintf(stderr,"Equating Recipes error occured\n");
		fprintf(stderr,"Source: %s at 2, Error: %s\n","er_ludcmp","singluar matrix");
		fprintf(stderr,"Exiting the system. Good bye!\n");
		exit(EXIT_FAILURE); 
	}
	else
...
{  
	...
	double max,                       /* max value on the current column */
		   temp;
	int    *pivot = (int *)malloc(n*sizeof(int));   /* vector for pivot  */
	double *b  = (double *)malloc(n*sizeof(double));/* temp. vector      */ 
 ...
 */
{
	...
	memset(diag,0,ncol2*sizeof(double));
	memset(coeff,0,ncol2*sizeof(double));
	for (k=1;k<=ncol2;k++) {
 ...
 */
{
	...
	double a,				 /* coefficient  of gk(lamda)                */
		   ...
		   test=0.0;
	double error = 2.0*pow(10.0,-6.0);		 /* tolerance level           */ 
	const double alpha = pow(10.0,-7.0);	 /* constant alpha in 9.7.7   */
	

	rcode = 0;								 /* normal exit               */   
	nrm2 = sqrt(er_dot(sk,sk,n,1));			 /* compute L2-norm of sk     */   
	if (nrm2 > maxstep)						 /* normalize sk vector       */ 
		er_scale(sk,maxstep/nrm2,sk,n,1);  
    dgx0 = er_dot(gk,sk,n,1); 
	for (i=1;i<=n;i++) 
	{
		temp = (fabs(xold[i])>1.0? fabs(xold[i]):1.0);
		temp = fabs(sk[i])/temp;
	...
	{ 
		memcpy(xnew+1,xold+1,n*sizeof(double));
		...
		if (lamda1 != 1.0)
		{
			...
			lamda1p2 = pow(lamda1,-2.0);
			lamda2p2 = pow(lamda2,-2.0);
 ...
 */
{
	...
	pk		= (double *)malloc((n+1)*sizeof(double));
	qk		= (double *)malloc((n+1)*sizeof(double));
	gk		= (double *)malloc((n+1)*sizeof(double));
	sk		= (double *)malloc((n+1)*sizeof(double));
	hddf	= (double *)malloc((n+1)*sizeof(double));
	xnew	= (double *)malloc((n+1)*sizeof(double));
	matxh	= (double **)allocate_matrix(DOUBLE,1,n,1,n);
 
	(*dftn_ptr)(xold,gk);
	/* set hessian matrix by identity matrix */
	for (i=1;i<=n;i++) matxh[i][i] = 1.0;   
	nrm2 = sqrt(er_dot(xold,xold,n,1));
	...
	{       
		...
		memcpy(sk+1,xnew+1,n*sizeof(double));         /* update pk       */
		er_daxpy(sk,-1.0,xold,n,1); 
		*fvalue  = (*ftn_ptr)(xold); 
		memcpy(xold+1,xnew+1,n*sizeof(double));       /* update xold     */ 
		memcpy(qk+1,gk+1,n*sizeof(double));          /* update qk and gk */ 
		er_scale(qk,-1.0,qk,n,1);                    /*     " "          */ 
		(*dftn_ptr)(xold,gk);                        /*     " "          */                        
		er_daxpy(qk,1.0,gk,n,1);                     /*     " "          */ 
		/* stopping condition */ 
		if ( sqrt(er_dot(sk,sk,n,1)) < pow(10.0,-8.0) ||
			 sqrt(er_dot(gk,gk,n,1)) < error ) 
			break;     
