...
*/
{
  ...
  fprintf(fp,"Equipercentile Equating with Single Group Design\n"
	  	     "and Polynomial Log-Linear Smoothing\n\n");
...
*/
{
  ...
  fprintf(fp,"Equipercentile Equating with CINEG Design"
             " (%s Anchor; w1 = %7.5f)\n"
			 "and Polynomial Log-Linear Smoothing\n\n",
 ...
 /* information about y and v in pop 2 */
  ...
  if(inall->method == 'F' || inall->method == 'G' || inall->method == 'A')
    fprintf(fp,"Braun-Holland (BH) Under Modified Frequency Estimation (MFE):\n"
               "    Intercept: b = %12.5f;     Slope: a = %12.5f\n\n",
               r->b[1], r->a[1]);
...
*/
{
  ...
  design_matrix(ns,min,inc, 0,0,0, c,0,0,NULL, scale,s->B_raw,s->B); 
  ...
  s->nit = iteration(fp, s->B, s->B_raw, fd, (double) n, 
                     NULL, ns, c, 0, 0, NULL,
  ...
  for (i=0;i<ns;i++)
    s->prd[i] = perc_rank(0, ns-1, 1, s->crfd, i);
...
*/
{
  ...
  fprintf(fp,"Univariate Log-Linear Smoothing using the Multinomial Model\n\n");

  fprintf(fp,"Number of score categories = %d\n",s->ns);
  fprintf(fp,"Number of persons (i.e., total of frequencies) = %7d\n\n",
	         s->num_persons);

  fprintf(fp,"Polynomial degree = %d\n\n",s->c);

  if(s->scale==0) fprintf(fp,"Design matrix B is powers of raw scores\n\n");
  else fprintf(fp,"Design matrix B is scaled powers of raw scores\n"
                  "such that for any column of B, sum(elements) = 0 and\n"
                  "sum(elements^2) = 1\n\n");
  
  fprintf(fp,"Convergence criterion:  ");
  if(s->Btype==0 && s->ctype==0)
    fprintf(fp,"|(n_mts[i] - m_mts[i]| <= %15.10f",s->crit); 
  else if(s->Btype==0 && s->ctype==1)
    fprintf(fp,"|n_mts[i] - m_mts[i])/n_mts[i]| <= %15.10f",s->crit);
  else if(s->Btype==1 && s->ctype==0)
    fprintf(fp,"|(n_mts_raw[i] - m_mts_raw[i]| <= %15.10f",s->crit); 
  else 
    fprintf(fp,"|n_mts_raw[i] - m_mts_raw[i])/n_mts_raw[i]|"
                 " <= %15.10f",s->crit);
  fprintf(fp,"  for i = 1 to %d\n\n",s->c);

  fprintf(fp,"Number of iterations to convergence = %d\n\n",s->nit);

  fprintf(fp,"Likelihood-ratio chi-square = %12.5f "
  ...
  if(s->scale==0) fprintf(fp,"Since scale=0, B_raw=B is the design matrix\n\n");

  /* print coefficients for design matrix */

  fprintf(fp,"COEFFICIENTS:\n\n");
    for(i=1;i<=s->c;i++)
      fprintf(fp,"   Beta[%2d] = %12.5f\n",i,s->Beta[i-1]); 
  ...
  if(print_dm && s->scale==1){ 
    ...
    fprintf(fp,"\n\nB DESIGN MATRIX");
    fprintf(fp,"\n\nCategory   Score");
    for(j=0;j<s->c;j++) fprintf(fp,"    Deg %2d",j+1); 
    fprintf(fp,"\n"); 
    for(i=0;i<s->ns;i++){
      fprintf(fp,"\n%8d%8.3f",i,score(i,s->min,s->inc));
      for(j=0;j<s->c;j++)
        fprintf(fp,"%10.5f",s->B[i][j]);
    }
  }
  ...
  if( (print_dm==2 && s->scale==1)  || (print_dm && s->scale==0) ){
    ...
    fprintf(fp,"\n\nB_raw DESIGN MATRIX");
    fprintf(fp,"\n\nCategory   Score");
    for(j=0;j<s->c;j++) fprintf(fp,"           Degree %2d",j+1);
    fprintf(fp,"\n"); 
    for(i=0;i<s->ns;i++){
      fprintf(fp,"\n%8d%8.3f",i,score(i,s->min,s->inc));
      for(j=0;j<s->c;j++)
        fprintf(fp,"%20.5f",s->B_raw[i][j]);
    }
  }
  ...
  fprintf(fp,"\n\n");
  for(i=1;i<=93;i++) fprintf(fp,"-");

  fprintf(fp,"\n\n                      Actual      Fitted\n");
  fprintf(fp,"                      freqs:      freqs:\n");
  fprintf(fp,"Category   Score       nct[]       mct[]          prop  fitted-prop"
	         "  fitted-crfd   fitted-prd\n\n");
  for(i=0;i<s->ns;i++)
    fprintf(fp,"%8d%8.3f%12.0f%12.5f       %7.5f %12.5f %12.5f %12.5f\n",
  ...
  if( (print_mts==2 && s->scale==1)  || (print_mts && s->scale==0) ){
    fprintf(fp,"\n");
    for(i=1;i<=s->c;i++)
      fprintf(fp,"     mts_raw[%2d]%12.5f%12.5f\n",
                 i,s->n_mts_raw[i-1], s->m_mts_raw[i-1]);  
  }

  if(print_mts) fprintf(fp,"\nNOTES:\n\n");
  ...
  if( (print_mts==2 && s->scale==1)  || (print_mts && s->scale==0) ){
    fprintf(fp,
      " mts_raw[] are based on no scaling of the design matrix. Importantly,\n" 
      " however, they are central moments associated with the design\n" 
      " matrix B_raw in the sense that mts_raw[1] = mean,\n"  
      " mts_raw[2] = sd, and for j = 3,...,%d, the jth central moment\n"   
      " is mts_raw[j] =  sum_i(B_raw[i][j-1] - mean)^(j)*rf[i]/sd^(j),\n" 
      " where mean = sum_i(B_raw[i][0]*f[i]).\n\n",s->c);
  }
  
  for(i=1;i<=70;i++)  fprintf(fp,"*");
...
*/
{
  ...
  s->nit = iteration(fp, s->B, s->B_raw, nct, (double) n, 
                     NULL, ns, cu, cv, cuv, cpm,
                     max_nit, ctype, Btype, crit,
                     s->Beta, s->mct, s->n_mts, s->m_mts, 
                     s->n_mts_raw, s->m_mts_raw, 
                     &(s->lrchisq),&(s->nzero), &(s->ap));     
...
*/
{
  ...
  if(s->anchor)
	fprintf(fp,"NOTE: In this output, variables are identified as\n"
	           "%c, %s, and %c, where %c = %s + %c; i.e., %c is the \n"
	           "total score, %s is the non-common-item score, and\n"
			   "%c is the common-item score.\n\n",
			   x,u,v,x,u,v,x,u,v);
  else
    fprintf(fp,"NOTE: In this output, variables are identified as %c and %c.\n"
	           "Since the anchor is external (explcitly if the design is CG;\n"
			   "implicitly if the design is SG), %c does not include %c\n\n",
			   x,v,x,v);
 
  fprintf(fp,"Number of score categories for %s  = %d\n",u,s->nsu);
  fprintf(fp,"Number of score categories for %c  = %d\n",v,s->nsv);
  fprintf(fp,"Total number of score categories = %d\n\n",s->ns);
  ...
  if(s->cuv>0){
    fprintf(fp,"Cross-Product moments: ");
    for(i=0;i<s->cuv;i++) fprintf(fp,"  (%s%d,%c%d)%c",
  ...
  if(print_dm && s->scale==1){
    ...
    fprintf(fp,"\n\nB DESIGN MATRIX");
    fprintf(fp,"\n\nCategory");
    for(j=0;j<s->nc;j++) fprintf(fp,"    Deg %2d",j+1); 
    fprintf(fp,"\n"); 
    for(i=0;i<s->ns;i++){
      fprintf(fp,"\n%8d",i);
      for(j=0;j<s->nc;j++)
  ...
  if( (print_dm==2 && s->scale==1)  || (print_dm && s->scale==0) ){
    ...
    fprintf(fp,"\n\nB_raw DESIGN MATRIX");
    fprintf(fp,"\n\nCategory");
    for(j=0;j<s->nc;j++) fprintf(fp,"           Degree %2d",j+1);
    fprintf(fp,"\n"); 
    for(i=0;i<s->ns;i++){
      fprintf(fp,"\n%8d",i);
      for(j=0;j<s->nc;j++)
  ...
  if(print_freq==1){                                /* vector format */
    fprintf(fp,"\n\n                      Actual      Fitted\n");
    fprintf(fp,"                      freqs:      freqs:\n");
    fprintf(fp,"Category               nct[]       mct[]\n\n");
    for(i=0;i<s->ns;i++)
      fprintf(fp,"%8d        %12.0f%12.5f\n",i,
              s->nct[i],s->mct[i]);
  }
  ...
  if(print_freq==2){                                /* matrix format */
    ...
    fprintf(fp,"\nCategory        ");
    for(j=0;j<s->nsv;j++) fprintf(fp,"   %c=%3d",v,j);
    fprintf(fp,"\n           Score");
    for(j=0;j<s->nsv;j++) fprintf(fp,"%8.3f",score(j,s->minv,s->incv));
    fprintf(fp,"\n");
  ...
  if(print_mts && s->scale==1){
    ...
    fprintf(fp,"\n\nMOMENTS\n");
    fprintf(fp,"                    Based on    Based on\n");
    fprintf(fp,"                      Actual      Fitted\n");
    fprintf(fp,"                       nct[]       mct[]");
  ...
  if(print_bfd==1){                                          /* print bfd[][] */
    ...
    for(i=0;i<s->nsx;i++){
      ...
      for(j=0;j<s->nsv;j++) fprintf(fp,"%8.3f",s->bfd[i][j]);
	}
  }
  ...
  if(print_bfd){  /* print row and col marginal freq, density, crfd, and prd */
    ...
    for(i=1;i<=17;i++) fprintf(fp," ");
	for(i=1;i<=51;i++) fprintf(fp,"-");
    fprintf(fp,"\nCategory   Score         Freq     Rel Freq    Cum RFreq"
	         "    Perc Rank\n\n");
    for(i=0;i<s->nsx;i++)
      fprintf(fp,"%8d%8.3f %12.5f %12.5f %12.5f %12.5f\n",
...
*/
{
  ...
  if(nsv==0){    
    for(i=0;i<nsu;i++){
      ...
      for(k=1;k<=cu;k++){ 
        B_raw[i][k-1] = pow(iscore, (double)k);
        B[i][k-1] = B_raw[i][k-1];
      }
    }
    goto SCALING;
  ...
  for(i=0;i<nsu;i++){
    ...
    for(j=0;j<nsv;j++){
      for(k=1;k<=cu;k++) 
        B_raw[cell][k-1] = pow(iscore, (double)k);
      cell++;
    }
  }
  ...
  for(j=0;j<nsv;j++){                          /* v polynomials */
    ...
    for(i=0;i<nsu;i++)
      for(k=1;k<=cv;k++) 
        B_raw[j + i*nsv][cu+k-1] = pow(jscore, (double)k);
  }
  ...
  for(j=0;j<ncols;j++){
    ...
    for(i=0;i<ncells;i++) B[i][j] = (B[i][j] - mn)/sqrt(ss); 
  }
...
*/
{
 ...
 /*  get right side of Eq 49 in H&T, which is computed using
  ...
  for(i=0;i<ns;i++) aloga += a[i]*log(a[i]);
  for(j=0;j<nc;j++){
    Baloga=0.; Ba=0.; 
    for(i=0;i<ns;i++){
      Baloga += B[i][j]*a[i]*log(a[i]);
	  Ba += B[i][j]*a[i]; 
    }
  ...
  ludcmp(left,nc,indx,&d);       
  lubksb(left,nc,indx,right);      
  */  
...
*/
{
  ...
  for(i=0;i<ns;i++){ 
	ap += (u[i] + BBeta[i] < ldmin) ? 0. : exp(u[i] + BBeta[i]);
  }
  ap = log(N) - ((ap < ldmin) ? 0. : log(ap));

       if(fp)
         fprintf(fp,"\n\nap = %12.5f",ap);

  for(i=0;i<ns;i++) 
    m[i] = (ap + u[i] + BBeta[i] < ldmin) ? 0. : exp(ap + u[i] + BBeta[i]);
...
{ 
  ...
  if(fp) Print_iteration_heading(fp, ns, nc, nct, n_mts, n_mts_raw,
                                 ctype, Btype, crit); 
  get_Beta0(B, nct, N, ns, nc, Beta, NULL);            /* initial Beta */ 
  ...
  for(nit=0;nit<=max_nit;nit++){ 
    ...
    *ap = get_mct(B, Beta, uin, N, ns, nc, mct, NULL); /* get ap & mct */
	/* ap is the normalizing constant which is needed in CLL */

    for(i=0;i<ns;i++)         
      if(nct[i]!=0. && mct[i]!=0.) *lrc += nct[i]*log(nct[i]/mct[i]);
      else (*nzero)++;         
    *lrc *= 2;                          /* likelihood-ratio chi-square */
    ...
    if(fp){                  /* print fitted-results for iteration nit */
      fprintf(fp,"\n        %2d    ",nit);
      fprintf(fp,"  %10.5f",ap);
      fprintf(fp,"  %10.5f",(uin==NULL) ? zero : uin[i]);
      for(i=0;i<nc;i++) fprintf(fp,"%12.3f",Beta[i]);
      for(i=0;i<nc;i++) fprintf(fp,"%12.5f",m_mts[i]);
      for(i=0;i<nc;i++) fprintf(fp,"%12.5f",m_mts_raw[i]);
      fprintf(fp,"%12.5f",*lrc);
      fprintf(fp,"%7d",*nzero);
      for(i=0;i<pcells;i++) fprintf(fp,"%12.5f",mct[i]);
      if(ns>250) fprintf(fp,"  ...");
    }
  ...
  if(Btype==0){            /* Btype == 0 --> based on B matrix */
    for(i=0;i<nc;i++){
      if(ctype==0){                     /* absolute comparison */
        if(fabs(n_mts[i] - m_mts[i]) > crit) return 0;
      }
      else{                             /* relative comparison */
        if(n_mts[i]==0.) 
          runerror("\n\nRelative criterion and n_mts[] = 0");
        if(fabs((n_mts[i] - m_mts[i])/n_mts[i]) > crit) return 0;
      }
    }
  }
...
*/
{
  int i,
    ...
    fprintf(fp,"\n\n              ");
    for(i=1;i<=(12*(nc+2));i++) fprintf(fp," ");

    for(i=1;i<=((12*nc)-18)/2;i++) fprintf(fp," ");
    fprintf(fp,"Moments Based on B");
    for(i=1;i<=((12*nc)-18)/2;i++) fprintf(fp," ");

    for(i=1;i<=((12*nc)-30)/2;i++) fprintf(fp," ");
    ...
    for(i=1;i<=(12*(nc+2));i++) fprintf(fp," ");
    fprintf(fp," ");
    for(i=1;i<=12*nc-1;i++) fprintf(fp,"-");
    fprintf(fp," ");
    for(i=1;i<=12*nc-1;i++) fprintf(fp,"-");
    ...
    fprintf(fp,"\n              ");
    for(i=1;i<=(12*(nc+2));i++) fprintf(fp," ");
    for(i=0;i<nc;i++) fprintf(fp,"       n[%2d]",i+1);
    for(i=0;i<nc;i++) fprintf(fp,"   n_raw[%2d]",i+1); 
    for(i=0;i<19;i++) fprintf(fp," ");    
    for(i=0;i<pcells;i++) fprintf(fp,"    nct[%3d]",i);
    if(ns>250) fprintf(fp,"  ...");
    fprintf(fp,"\n\n        Actual");
    for(i=1;i<=(12*(nc+2));i++) fprintf(fp," ");
    for(i=0;i<nc;i++) fprintf(fp,"%12.5f",n_mts[i]);
    for(i=0;i<nc;i++) fprintf(fp,"%12.5f",n_mts_raw[i]);
    for(i=0;i<19;i++) fprintf(fp," ");
    for(i=0;i<pcells;i++) fprintf(fp,"%12.5f",nct[i]);
    if(ns>250) fprintf(fp,"  ...");
    ...
    fprintf(fp,"          ap");
    fprintf(fp,"           u");
    for(i=0;i<nc;i++) fprintf(fp,"    Beta[%2d]",i+1);
    for(i=0;i<nc;i++) fprintf(fp,"       m[%2d]",i+1);
    for(i=0;i<nc;i++) fprintf(fp,"   m_raw[%2d]",i+1);
    fprintf(fp,"     LR Chi2");
    fprintf(fp,"  nzero");
    for(i=0;i<pcells;i++) fprintf(fp,"    mct[%3d]",i);
    if(ns>250) fprintf(fp,"  ...");
    fprintf(fp,"\n");
