...
*/
{
  ...
  if(inall->rep == 0){      /* no assignment or storage alloc for bootstrap reps */
    strcpy(inall->xfname,xtoy->xfname);
    strcpy(inall->yfname,xtoy->yfname);
    inall->design = design;
...
*/
{
  ...
  int i,
      ...
      low,                                     /* lowest score with pr >= prlow */
	  high,                                  /* highest score with pr <= prhigh */
  ...
  case 'C':
		...
		if(z->method == 'C'){                                           /* chained */
			...
			for (i=0;i<ns;i++) prd[i] = perc_rank(0,ns-1,1,Fxs, (double) i);
			for(i=0; i<ns; i++) 
  ...
  if(inverse==0)
  {
     postSmooth(raw, vt_scores, vt_stderr, ns, s, low, high, 
               (double) (ns2-1), raw, ns, vt_eeqs, mx_coeff);
  } 
  else{
    ...
    inversePostSmooth(raw, vt_scores, vt_stderr, ns, s, low, high,
	                  (double)(ns2-1), rawx, ns2, vt_inverse, mx_coeff);
    ...
    ch = (vt_eeqs[high] - ((ns2-1) + 0.5))/(high - ((ns-1) + 0.5));
  ...
  for (i=0; i<nsb;i++) 
  {
	  ...
	  cs->cmat[3*nsb+i] = mx_coeff[3*nsb+i]/pow(inc,2.0); 
  }
  if(inverse==1) 
  {
	  for (i=0; i<ns2 ;i++) 
...
*/
{
  ...
  if(inall->rep>0)  fprintf(fp,"Cubic-spline postsmoothing %d\n\n",inall->rep);
  else fprintf(fp,"Cubic-Spline Postsmoothing ");
  
  if(inall->design == 'R') 
    fprintf(fp,"Based on Random Groups Design");
  else if(inall->design == 'S') 
    fprintf(fp,"Based on Single Group Design");
  else{  
    fprintf(fp,"Based on CINEG Design");
	fprintf(fp,"\nUsing Method = %c =",xtoy->method);
	if(xtoy->method=='E') fprintf(fp," Frequency Estimation\n");
	else if(xtoy->method=='F') fprintf(fp," Modified Frequency Estimation\n");
	else if(xtoy->method=='C') fprintf(fp," Chained Equipercentile\n");
	else runerror("\nShould not get here.");
  }
  ...
  if(parm==2){
    fprintf(fp,"Putting Y on scale of X\n\n");	  
...
*/
{
  ...
  fprintf(fp,"s = %9.5f\n\n",z->cs->s);
  fprintf(fp,"Number of raw-score categories = %d\n",z->cs->ns);
  fprintf(fp," prlow = %9.5f;  low = %9.5f (in location %4d)\n",
	         z->cs->prlow, sc_low, low);
  fprintf(fp,"prhigh = %9.5f; high = %9.5f (in location %4d)\n",
  ...
  fprintf(fp,"%9.5f %9.5f %9.5f                               %9.5f %9.5f\n",
	...
	z->cs->se[high],
	pow( ((z->cs->eeqs[high]-z->cs->eeq[i])/z->cs->se[i]), 2.));
...
*/
{
	...
	if (mx == NULL)
	{
		printf("Error, dpdch: Input Error \n\tnull pointer\n");
		exit(-1);
	}
	for (i=0 ; i < dim ; i++ )
	{
...
*/
{
	...
	if ( tdm1 == NULL || tdm2 == NULL )
	{
		printf("Error, dtdmt: Input error \n\tnull pointer\n");
		exit(-1);
	}

	/* initialize tdm2 by 0.0*/
	memset(tdm2,0,rdim1*cdim1*sizeof(double));
...
*/
{
	...
	if ( tdm2 == NULL || tdm1 == NULL )
	{
		printf("Error,dtdsm: Input error \n\tNull pointer\n");
		exit(-1);
	}
	/* initialize matrix tdm2 by 0.0		*/
	memset(tdm2,0,rdim1*cdim1*sizeof(double));
	if ( c==1.0 )
		memcpy(tdm2,tdm1,rdim1*cdim1*sizeof(double));
	else if (c==0.0)
		memset(tdm2,0,rdim1*cdim1*sizeof(double));
	else
		...
		for (i=0 ; i < rdim1 ; i++ )
		{
			 ...
			 *  operations over zero values in tdm3      * 
			 *  left1 : left-end of non-zeros in tdm1    * 
			 *  right1: right-end of non-zeros in tdm1   */
			left 	= (i-2>=0? i-2: 0);    
...
*/
{
	...
	if (tdm1 == NULL || tdm2 == NULL || tdm3 == NULL)
	{
		printf("Error, dtdmm: Input Error \n\tnull pointer\n");
		exit(-1);
	}
	/* initialize tdm3 by 0.0       */
	memset(tdm3,0,rdim1*cdim2*sizeof(double));
...
*/
{
	...
	if (tdm1 == NULL || tdm2 == NULL || tdm3 == NULL)
	{
		printf("Error, dtdma: Input Error \n\tnull pointer\n");
		exit(-1);
	} 
	/* compute tdm3 = tdm1 + tdm2 */
	memset(tdm3,0,rdim1*cdim1*sizeof(double));
	for (i=0 ; i < rdim1 ; i++ )
	{
...
{  
	...
	if ( vx==NULL|| vb==NULL|| mpd==NULL )
	{
		printf("Error, chsol: Input Error\nnull pointer \n");
		exit(-1);
	}
	/* cholesky decomposes mpd		*/
	memcpy(vx,vb,dim*sizeof(double));
...
*/
{
	...
	if (v1 == NULL || v2 == NULL || mtd == NULL)
	{
		printf("Error, dtdmm: Input Error \n\tnull pointer\n");
		exit(-1);
	}  
	/* calculate v2 = mtd * v1		*/
	memset(v2,0,rdim*sizeof(double)); 
	for (i=0 ; i < rdim ; i++ )
	{
		/*  left and right help to skip operations over zero 
		    values in mtd */
...
*/
{
	...
	slope = (y1-y0)/(x1-x0) ;
	/* check that slope != 0         */
 	if (fabs(slope)<pow(10.0,3.0)*DBL_EPSILON) 
		printf("linearPoly: Input Warning, slope ~ 0 \n"); 
...
*/
{
	...
	{
		printf("cubicPoly: Input Error, cubic poly. used outside domain\n");
		printf("xleft = %f, xright= %f xvalue = %f\n", xleft, xright, xvalue);
	}
	/* evaluate the function         */
	tempX  = xvalue-xleft;
	rvalue = ai+bi*tempX+ci*pow(tempX,2.0)+di*pow(tempX,3.0); 
	return rvalue;
}
...
*/
{
	...
	if ( mt == NULL )
	{
		printf("Error: setup_matrixT \n\tnull pointer\n");
		exit(-1);
	}

	/* initialize all elements in matrix mt(,) by 0.0  */
	memset(mt,0,(n-1)*(n-1)*sizeof(double));
...
*/
{
	...
	memset(mqt,0,(n+1)*(n-1)*sizeof(double));
...
*/
{
	 ...
	 if (x==NULL || y==NULL || dyi==NULL ||mc==NULL)
	 {
		printf("Error, sspline: Input Error \n\tnull pointer\n");
		return 0;
	 }
	 n=num-1;	                          
	 s		= s*(x[n]-x[0]+1);  
	 /* printf("(x[n]-x[0]+1) = %f \t s = %f \n",(x[n]-x[0]+1),s); */ 
	 va		= (double *)malloc((n+1)*sizeof(double));
	 vc		= (double *)malloc((n+1)*sizeof(double));
	 vu		= (double *)malloc((n-1)*sizeof(double));
	 vw		= (double *)malloc((n-1)*sizeof(double));
	 vtw	= (double *)malloc((n-1)*sizeof(double)); 
	 vh		= (double *)malloc(n*sizeof(double));
	 vv		= (double *)malloc((n+1)*sizeof(double));
	 vy2	= (double *)malloc((n+1)*sizeof(double));	    
	 vtu	= (double *)malloc((n-1)*sizeof(double)); 	    
	 mq		= (double *)malloc((n-1)*(n+1)*sizeof(double));          
	 mt		= (double *)malloc((n-1)*(n-1)*sizeof(double));         
	 mpt	= (double *)malloc((n-1)*(n-1)*sizeof(double));
	 mqt	= (double *)malloc((n-1)*(n+1)*sizeof(double));        
	 mpt1	= (double *)malloc((n-1)*(n-1)*sizeof(double));  
	 mpt2	= (double *)malloc((n-1)*(n-1)*sizeof(double));  
	 mtmp	= (double *)malloc((n-1)*(n+1)*sizeof(double));  
 ...
 */
{
	...
	for (i=0; i< num2; i++)
	{
		...
		{
			printf("postSmooth: Input Error\n");
			printf("xvalue = %8.5f xvalues[xlow]=%8.5f xvalues[xhigh]=%8.5f \n", 
...
{                  
	double diff,									   /* |right - left| */
		   ...
		   mid;                    /* middle point of left &right        */ 
	const double ERROR=pow(10.0,6.0)*DBL_EPSILON;
	...
	if ( side1 >= side2 )
	{
		printf("inverseCubicPoly: Input Error 1\n");
		printf("left end = %f right end = %f\n", side1, side2);
		return yvalue;
	} 
	if ( yvalue < side1 || side2 <yvalue ) 
	{
		printf("inverseCubicPoly: Input Error 2\n");
		printf("left end=%f right end=%f yvalue=%f\n", side1, side2, yvalue);
		return yvalue;
	}  
 ...
 */
{
	...
	xnodes=(double *)malloc((n2+1)*sizeof(double));
	for(j=0; j<n2; j++)
	...
	for (i=0; i< n1; i++)
	{
		...
		{
			printf("inverseSSpline: Input Error\n");
			printf("value = %f\n", vectX[i]);
			return;
		}
