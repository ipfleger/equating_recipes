...
*/
{
  ...
  if(rv1 != 0 && rv2 != 0){
    ...
    for(i=0;i<nsv;i++) muv1 += i*h1[i];
    for(i=0;i<nsv;i++) muv2 += i*h2[i];
...
*/
{
  ...
  MomentsFromRFD(minx,maxx,inc,NULL,fxs,mtsxs); 
  MomentsFromRFD(miny,maxy,inc,NULL,gys,mtsys);
...
*/
{
  double v1, /* non integer v in pop 1 associated with integer v in pop 2 */
         slope = sqrt(rv2/rv1),                          /* slope for MFE */
         intercept = ((1-sqrt(rv2))/sqrt(rv1))*muv2 - 
                     ((1-sqrt(rv1))/sqrt(rv1))*muv1, /* intercept for MFE */
...
*/
  ...
  if(inall->method == 'E' || inall->method == 'G' || inall->method == 'A'){
    fprintf(fp,"\n\n%s\n\n",tt);
    fprintf(fp,"Synthetic Densities and Equated Scores Under\n"
               "Frequency Estimation (w1 = %7.5f)\n\n",inall->w1);
    fprintf(fp,"Input file for xv and pop 1: %s\n",inall->xfname);
    fprintf(fp,"Input file for yv and pop 2: %s\n\n",inall->yfname);
    fprintf(fp,"      Raw (x)           fxs");
    fprintf(fp,"      Raw (y)           gys");
    fprintf(fp,"          FE\n");

    for(i=0;i<inall->xv->ns1;i++){
      fprintf(fp,"\n %12.5f  ",score(i,inall->xv->min1,inall->xv->inc1));
      fprintf(fp,"%12.9f",r->fxs[0][i]);
      if(i<inall->yv->ns1){
        fprintf(fp," %12.5f  ",score(i,inall->yv->min1,inall->yv->inc1));
        fprintf(fp,"%12.9f",r->gys[0][i]);
      }
      else{
        fprintf(fp,"                            ");
      }
      fprintf(fp,"%12.5f",r->eraw[0][i]);
    }
    
    while(inall->yv->ns1>i){
      fprintf(fp,"               ");
      fprintf(fp,"\n %12.5f  ",score(i,inall->yv->min1,inall->yv->inc1));
      fprintf(fp,"%12.5f",r->gys[0][i]);
      i++;  
    }
    ...
    fprintf(fp,"\n\n       Mean ");
    fprintf(fp,"%15.9f            %15.9f",mtsfxs[0],mtsgys[0]);
    fprintf(fp,"\n       S.D. ");
    fprintf(fp,"%15.9f            %15.9f",mtsfxs[1],mtsgys[1]);
    fprintf(fp,"\n       Skew ");
    fprintf(fp,"%15.9f            %15.9f",mtsfxs[2],mtsgys[2]);;
    fprintf(fp,"\n       Kurt ");
    fprintf(fp,"%15.9f            %15.9f",mtsfxs[3],mtsgys[3]);
    fprintf(fp,"\n\nSum of RF's ");
    fprintf(fp,"%15.9f            %15.9f",fxssum,gyssum);
  ...
  if(inall->method == 'F' || inall->method == 'G' || inall->method == 'A'){
    ...
    for(i=0;i<inall->xv->ns1;i++){
      fprintf(fp,"\n %12.5f  ",score(i,inall->xv->min1,inall->xv->inc1));
      fprintf(fp,"%12.9f",r->fxs[1][i]);
      if(i<inall->yv->ns1){
        fprintf(fp," %12.5f  ",score(i,inall->yv->min1,inall->yv->inc1));
        fprintf(fp,"%12.9f",r->gys[1][i]);
      }
      else{
        fprintf(fp,"                            ");
      }
      k = (inall->method == 'F') ? 0 : 2;
      fprintf(fp,"%12.5f",r->eraw[k][i]);
    }
    
    while(inall->yv->ns1>i){
      fprintf(fp,"               ");
      fprintf(fp,"\n %12.5f  ",score(i,inall->yv->min1,inall->yv->inc1));
      fprintf(fp,"%12.5f",r->gys[1][i]);
      i++;  
    }
